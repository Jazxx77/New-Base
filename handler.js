        ...
        try {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (!isString(user.planetName)) user.planetName = '';
                    if (!isNumber(user.planetAge)) user.planetAge = 0;
                    if (!isString(user.planetWeather)) user.planetWeather = '';
                    if (!Array.isArray(user.planetEcosystem)) user.planetEcosystem = [];
                    if (!Array.isArray(user.planetSpecies)) user.planetSpecies = [];
                    if (typeof user.resources !== 'object' || user.resources === null) user.resources = { crystal: 0, biolight: 0, plasma: 0, toxin: 0, frostshard: 0 };
                    if (!isNumber(user.planetChallengesCompleted)) user.planetChallengesCompleted = 0;
                    if (!isNumber(user.planetEvolutionPoints)) user.planetEvolutionPoints = 0;
                    if (!isNumber(user.planetDefenseLevel)) user.planetDefenseLevel = 0; // Level pertahanan untuk pertempuran di masa depan
                    if (typeof user.customAttributes !== 'object' || user.customAttributes === null) user.customAttributes = {}; // Atribut kustomisasi
                    if (!Array.isArray(user.randomEvents)) user.randomEvents = []; // Pastikan ini selalu diinisialisasi sebagai array
                    if (!isNumber(user.planetRanking)) user.planetRanking = 0; // Peringkat di papan leaderboard
                    ...
                    planetChallengesCompleted: 0,
                    planetEvolutionPoints: 0,
                    planetDefenseLevel: 0, // Level pertahanan untuk pertempuran di masa depan
                    customAttributes: {}, // Atribut kustomisasi
                    randomEvents: [], // Pastikan ini selalu diinisialisasi sebagai array
                    planetRanking: 0, // Peringkat di papan leaderboard
...
title: `© Interindah - Assistant V1.0.0`,
thumbnailUrl: 'https://files.catbox.moe/xczqj5.jpg',
mediaType: 1,
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
						...
						try {
                            ...
                            title: '© ᴠʏɴᴢᴢʙᴏᴛ',
                            thumbnailUrl: pp,
                            sourceUrl: 'https://wa.me/+6282121122008',
                            mediaType: 1,
                            renderLargerThumbnail: true
                            }}}, {quoted: null})
                        }
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.reply(m.chat, msg, m)    
        let msgg = {
            unreg: 'ʜᴇʟʟᴏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ ᴜꜱɪɴɢ ᴛʜᴇ ʙᴏᴛ, ᴘʟᴇᴀꜱᴇ ʀᴇɢɪꜱᴛᴇʀ ꜰɪʀꜱᴛ.\nʜᴏᴡ ᴛᴏ ʀᴇɢɪꜱᴛᴇʀ?\n\n> ʜᴀʟᴏ ᴜɴᴛᴜᴋ ᴛᴇʀᴜꜱ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ʙᴏᴛ, ꜱɪʟᴀᴋᴀɴ ᴍᴇɴᴅᴀꜰᴛᴀʀ ᴛᴇʀʟᴇʙɪʜ ᴅᴀʜᴜʟᴜ.\n> ʙᴀɢᴀɪᴍᴀɴᴀ ᴄᴀʀᴀ ᴍᴇɴᴅᴀꜰᴛᴀʀ?\n\n*ᴇxᴀᴍᴘʟᴇ:* .ʀᴇɢɪꜱᴛᴇʀ'
         }[type]
         if (msgg) return conn.sendUrlImageButton(m.chat, msgg, [{name: "quick_reply", buttonParamsJson: `{"display_text": "Register", "id": ".register"}`}], wm, "https://8030.us.kg/file/yK3xrVyzlPgX.jpg", m)
    let msg3 = {
